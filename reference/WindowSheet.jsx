/*
  WindowSheet.jsx — Milla Homes printable A4 "window sheet".

  Framework-agnostic React component (no proprietary runtime). Drop into the
  Next.js repo as-is. One component, three presets.

      <WindowSheet property={listing} preset="poster" />   // "poster" | "gallery" | "boutique"

  Sizing: each sheet is 794 × 1123 px = 210 × 297 mm at 96dpi (A4). For PDF/print use:
      @page { size: A4; margin: 0 }
  and render the sheet at its natural size (no transform). For on-screen preview, wrap in a
  scaled container, e.g. style={{ transform: `scale(${w/794})`, transformOrigin: "top left" }}.

  Fonts: load Cormorant Garamond + Mulish (Google Fonts) globally.
  Logo: pass logoDark / logoLight (defaults assume /public/logos/...). logoLight is the
  cream-tinted version used on the deep-green band in the "gallery" preset.

  QR: this file uses `window.qrcode` (the qrcode-generator UMD global) so it runs without a
  bundler. ===> In the repo, replace the getQrcode() helper with:  import qrcode from "qrcode-generator";
*/
...
// ---- QR ------------------------------------------------------------
function getQrcode() {
  // repo: `import qrcode from "qrcode-generator"` and just `return qrcode;`
  return typeof window !== "undefined" ? window.qrcode : null;
}
function useQR(url, color) {
  const ref = useRef(null);
  useEffect(() => {
    let alive = true;
    const draw = () => {
      const qrcode = getQrcode();
      if (!qrcode) { setTimeout(draw, 120); return; }
      ...
      for (let r = 0; r < n; r++)
        for (let c = 0; c < n; c++)
          if (qr.isDark(r, c))
            cells += `<rect x="${c}" y="${r}" width="1.04" height="1.04"></rect>`;
      ref.current.innerHTML =
        `<svg viewBox="0 0 ${n} ${n}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" shape-rendering="crispEdges" fill="${color}">${cells}</svg>`;
    };
    draw();
    return () => { alive = false; };
  }, [url, color]);
...
function IconBed({ size = 22, stroke = C.goldInk }) {
  return (<svg {...ic(size, 1.4)} stroke={stroke}><path d="M2 17v-4a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v4" /><path d="M2 17h20" /><path d="M2 17v3M22 17v3" /><path d="M5 11V8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v3" /></svg>);
}
function IconBath({ size = 22, stroke = C.goldInk }) {
  return (<svg {...ic(size, 1.4)} stroke={stroke}><path d="M5 12V6.5a2.5 2.5 0 0 1 5 0" /><path d="M3 12h18v2a5 5 0 0 1-5 5H8a5 5 0 0 1-5-5z" /><path d="M7 19l-1.5 2M17 19l1.5 2" /></svg>);
}
function IconArea({ size = 22, stroke = C.goldInk }) {
  return (<svg {...ic(size, 1.4)} stroke={stroke}><path d="M3 8V3h5" /><path d="M21 8V3h-5" /><path d="M3 16v5h5" /><path d="M21 16v5h-5" /></svg>);
}
function IconPhone({ size = 15, stroke = C.goldInk }) {
  return (<svg {...ic(size, 1.6)} stroke={stroke}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.34 1.85.57 2.81.7A2 2 0 0 1 22 16.92z" /></svg>);
}
function IconGlobe({ size = 15, stroke = C.goldInk }) {
  return (<svg {...ic(size, 1.6)} stroke={stroke}><circle cx="12" cy="12" r="9" /><path d="M3 12h18M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18" /></svg>);
}

// ---- helpers -------------------------------------------------------
const nb = (s) => (s || "").replace(/\s+/g, "\u00A0"); // keep price on one line
...
// ================================================================
function Gallery({ p, logoLight }) {
  const photos = p.photos || [];
  const qr = useQR(p.listingUrl, C.text);
  const chip = { display: "flex", alignItems: "center", gap: 9, background: C.surfaceStrong, border: `1px solid ${C.border}`, borderRadius: 3, padding: "11px 16px", color: C.text, fontSize: 15, fontWeight: 700 };
  const chipLabel = { fontWeight: 500, color: C.muted };
  return (
...
// ================================================================
function Boutique({ p, logoDark }) {
  const photos = p.photos || [];
  const qr = useQR(p.listingUrl, C.text);
  const divider = <div style={{ width: 1, height: 22, background: "rgba(18,53,36,.18)" }} />;
  const spec = { display: "flex", alignItems: "center", gap: 8, fontSize: 15, fontWeight: 600, padding: "0 22px" };
  return (
...
// UMD-ish exposure so this file can also be loaded without a bundler (e.g. preview harness).
if (typeof module !== "undefined" && module.exports) module.exports = { WindowSheet, PRESETS };
if (typeof window !== "undefined") { window.WindowSheet = WindowSheet; window.WindowSheetPresets = PRESETS; }
